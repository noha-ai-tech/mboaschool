...
import { ADDITIONAL_FEE_CATEGORIES, type SchoolPagePricing } from "@/lib/schoolPage/pricing";
...
export function StructuredPricingEditor({ value, onChange }: { value: SchoolPagePricing; onChange: (value: SchoolPagePricing) => void }) {
  ...
  return <div className="space-y-5">
    ...
    <section className="space-y-3">
      <div className="flex items-center justify-between"><h3 className="text-sm font-bold">Tarifs par niveau</h3><button type="button" onClick={() => update({ schedules: [...value.schedules, { academic_year: "", level_label: "", registration_fee: null, tuition_fee: null, currency: value.currency, notes: null, position: value.schedules.length, installments: [] }] })} className="text-xs font-bold text-primary inline-flex gap-1"><Plus size={14}/>Ajouter</button></div>
      {value.schedules.map((schedule, index) => <div key={index} className="space-y-2 rounded-xl border border-border p-3">
        <div className="grid grid-cols-2 gap-2"><input className={field} placeholder="Année 2026-2027" value={schedule.academic_year} onChange={(e) => updateSchedule(index, { academic_year: e.target.value })}/><input className={field} placeholder="Niveau / classe" value={schedule.level_label} onChange={(e) => updateSchedule(index, { level_label: e.target.value })}/></div>
    ...
    <section className="space-y-3">
      ...
      {value.additional_fees.map((fee, index) => <div key={index} className="grid grid-cols-2 gap-2 rounded-xl border border-border p-3">
        ...
        <input className={field} placeholder="Libellé" value={fee.label} onChange={(e) => updateAdditional(index, { label: e.target.value })}/><input className={field} type="number" min={0} placeholder="Montant" value={fee.amount} onChange={(e) => updateAdditional(index, { amount: Number(e.target.value) })}/>
        <input className={field} placeholder="Fréquence" value={fee.frequency} onChange={(e) => updateAdditional(index, { frequency: e.target.value })}/><label className="flex items-center gap-2 text-xs"><input type="checkbox" checked={fee.mandatory} onChange={(e) => updateAdditional(index, { mandatory: e.target.checked })}/>Obligatoire</label>
