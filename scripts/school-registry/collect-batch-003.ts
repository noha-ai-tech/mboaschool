...
/**
 * SPRINT Q — Batch 003. Collecte réelle (réseau live, aucune fixture) du
 * répertoire ESG MINESEC pour les 4 régions manquantes : Sud, Est,
 * Nord-Ouest, Sud-Ouest — complète la couverture nationale.
 *
 * Pipeline entièrement réutilisé de SPRINT N/O (Batch 001/002) sans
 * modification : même collecteur (`fetchEsgByRegion`), même normalizer,
 * même dédoublonneur. Seule la liste de régions change. Valeurs de filtre
 * ("SUD", "EST", "NORD-OUEST", "SUD-OUEST") vérifiées en direct contre les
 * <option> réelles du formulaire MINESEC avant d'écrire ce script — pas
 * devinées.
 *
 * N'écrit QUE dans data/registry/{raw,normalized}/ (fichiers locaux). Aucun
 * appel Supabase, aucune écriture staging/establishments.
 *
 * Usage : node_modules/.bin/tsx collect-batch-003.ts
 */

const REGIONS = ["SUD", "EST", "NORD-OUEST", "SUD-OUEST"] as const;
...
async function main() {
  ...
  const rawPath = join(rawDir, `minesec-sud-est-anglophone-${today}.json`);
  ...
  const normalizedPath = join(normalizedDir, `minesec-sud-est-anglophone-${today}.json`);
...
main().catch((error) => {
  console.error("Échec de la collecte Batch 003 :", error);
  process.exit(1);
});
