...
test("l’aperçu reste privé, contextualisé et utilise le renderer public existant", async () => {
  const preview = await read("src/app/dashboard/ecole/etablissement/preview/page.tsx");
  const publicPage = await read("src/app/ecole/[id]/page.tsx");
  assert.match(preview, /fetch\("\/api\/school-page\/preview"\)/);
  assert.match(preview, /buildSchoolPageSections/);
  assert.match(preview, /withEstablishmentQuery/);
  assert.match(preview, /Cette version n&apos;est pas encore publique/);
  assert.doesNotMatch(preview, /\.from\("applications"\)/);
  assert.match(publicPage, /buildSchoolPageSections/);
});
...
test("les surfaces plein écran compensent exactement le padding mobile du shell", async () => {
  const [editor, preview] = await Promise.all([
    ...
    read("src/app/dashboard/ecole/etablissement/preview/page.tsx"),
  ]);
