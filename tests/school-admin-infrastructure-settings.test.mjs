...
test("Paramètres conserve sa liste blanche et protège les champs sensibles", async () => {
  ...
  // RELEASE-CONSOLIDATION-02 §5D — phone/email/website/description/address/
  // city are governed exclusively by the Draft/Publish CMS and protected by
  // a DB trigger (migration 0035): this page's EMPTY_FORM must NOT include
  // them, or a save would raise 42501 and take name/neighborhood/whatsapp/
  // main_category down with it in the same statement.
  for (const field of ["name", "neighborhood", "whatsapp", "main_category"]) assert.match(form, new RegExp(`${field}:`));
  for (const field of ["city", "phone", "email", "website", "description", "address", "official_id", "source_ministry", "source_reference", "registry_import_batch", "is_verified", "forfait", "subscription_plan", "owner_id", "organization_id"]) assert.doesNotMatch(form, new RegExp(`${field}:`));
